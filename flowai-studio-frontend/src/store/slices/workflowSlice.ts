...
export interface WorkflowSlice {
  ...
  cancelWorkflowExecution: (workflowId: string) => Promise<void>
  ...
  streamRunWorkflow: async (workflowId, inputs) => {
    ...
    try {
        ...
        if (event.type === 'event') {
          try {
            ...
            if (data.type === 'workflow_start') {
              ...
              set({ executionStatus: 'success', activeExecutionId: null })
      ...
      if (error instanceof DOMException && error.name === 'AbortError') {
        set({ executionStatus: 'stopped', activeExecutionId: null })
        return
      }
      set({ executionStatus: 'failed', activeExecutionId: null })
      throw error
    } finally {
      if (activeWorkflowController === controller) {
        activeWorkflowController = null
      }
    }
  },

  cancelWorkflowExecution: async (workflowId) => {
    const executionId = get().activeExecutionId
    try {
      if (executionId) {
        await request.post(
          `/workflows/${workflowId}/cancel/${encodeURIComponent(executionId)}`,
        )
      }
    } finally {
      activeWorkflowController?.abort()
