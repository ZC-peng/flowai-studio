...
const ConfigPanel: React.FC = () => {
  ...
  useEffect(() => {
    fetchKnowledgeBases()
    fetchSkills()
    request.get('/skill/builtin/list')
      .then((response: any) => setBuiltinTools(Array.isArray(response.data) ? response.data : []))
      .catch(() => setBuiltinTools([]))
    request.get('/mcp/tools')
      .then((response: any) => setMcpTools(Array.isArray(response.data) ? response.data : []))
      .catch(() => setMcpTools([]))
    ...
    return (
      <>
        ...
        <Form.Item name="toolIds" label="可用工具">
          <Select mode="multiple" placeholder="选择 Agent 可调用的工具（留空表示不授权工具）">
            <OptGroup label="内置工具">
              {builtinTools.map(tool => (
                <Option key={`builtin:${tool.type}`} value={`builtin:${tool.type}`}>{tool.name}</Option>
              ))}
            </OptGroup>
            <OptGroup label="自定义工具">
              {Array.isArray(skills) && skills.map(s => (
                <Option key={s.id} value={s.id}>{s.name}</Option>
              ))}
            </OptGroup>
            <OptGroup label="已连接的 MCP 工具">
              {mcpTools.map(tool => (
                <Option
                  key={`mcp:${tool.serverId}:${tool.name}`}
