 ...
 *
 * 设计理念:
 * - Single ReAct 作为核心执行模式
 * - Supervisor/Worker 作为实验性扩展路径
...
export interface AgentMessage {
  ...
  /** 工具调用信息 */
  toolCalls?: ToolCall[];
  /** tool 消息对应的标准 Function Calling ID */
  toolCallId?: string;
  /** tool 消息对应的工具名称 */
  toolName?: string;
  /** 时间戳 */
...
export interface AgentExecutionResult {
  ...
  /** RAG 调用统计 */
  ragCallCount: number;
  /** 结构化知识库引用，供工作流结果与前端展示 */
  ragResults?: RAGResult[];
  /** 总迭代次数 */
  iterations: number;
  /** 所有 LLM 轮次累计 Token 用量 */
  tokenUsage: {
    promptTokens: number;
    completionTokens: number;
    totalTokens: number;
