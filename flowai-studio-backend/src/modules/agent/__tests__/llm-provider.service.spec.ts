...
describe('QwenProvider', () => {
  ...
  describe('chat', () => {
    it('should fail locally when the API key is missing', async () => {
      const unconfigured = new QwenProvider({ apiKey: '' });
    ...
    it('should preserve tool_calls and tool_call_id in the next request', async () => {
      mockedAxios.post.mockResolvedValue({
        data: {
          choices: [{ message: { content: '结果是 2' }, finish_reason: 'stop' }],
          model: 'qwen-turbo',
        },
      });
