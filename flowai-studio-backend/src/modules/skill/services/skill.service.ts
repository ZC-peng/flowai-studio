...
import { GIS_SKILL_DEFINITIONS } from '../utils/gis-skills';
...
export class SkillService {
  ...
  // 执行工具
  // 内置工具使用 builtin:<type> 稳定 ID；自定义工具必须携带用户上下文。
  async executeSkill(skillId: string, params: Record<string, any>, userId?: string) {
    if (skillId.startsWith('builtin:')) {
      return executeBuiltinSkill(skillId.slice('builtin:'.length), params);
    }

    if (!userId) {
      throw new BadRequestException('User context is required to execute a custom skill');
    }
