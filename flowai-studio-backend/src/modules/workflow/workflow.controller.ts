...
export class WorkflowController {
  ...
  async streamRun(
    ...
    // workflow_traces.executionId is a PostgreSQL UUID column. Keep the
    // runtime cancellation key and the persisted trace identifier identical.
  ...
  async cancelExecution(
    ...
    await this.workflowService.findOne(userId, id);
    const cancelled = this.workflowExecutorService.cancelExecution(
      executionId,
      userId,
      id,
    );
  ...
  async getRunningExecutions(
    ...
    await this.workflowService.findOne(userId, id);
    return {
      executions: this.workflowExecutorService.getRunningExecutions(userId, id),
    };
  }
}
