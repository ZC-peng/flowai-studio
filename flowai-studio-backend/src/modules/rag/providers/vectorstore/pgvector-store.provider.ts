...
export class PgVectorStore implements VectorStore {
  ...
  async upsert(collectionName: string, documents: VectorDocument[]): Promise<void> {
    ...
    // document_chunks 同时承担业务分块表与 pgvector 存储，必须保留文档外键和分块位置。
    const invalidDocument = documents.find((doc) => !doc.metadata?.documentId);
    if (invalidDocument) {
      throw new Error('PgVector document metadata.documentId is required');
    }
    ...
    const values = documents.map((doc) => {
      ...
      const metadataValue = doc.metadata
        ...
        : 'NULL';
      const chunkIndex = Number(doc.metadata?.chunkIndex ?? 0);
      const startIndex = Number(doc.metadata?.startIndex ?? 0);
      const endIndex = Number(doc.metadata?.endIndex ?? doc.content.length);
      const documentId = String(doc.metadata?.documentId).replace(/'/g, "''");
    ...
    for (let i = 0; i < values.length; i += batchSize) {
      ...
      await this.prisma.$executeRawUnsafe(`
        INSERT INTO document_chunks
          (content, embedding, "chunkIndex", "startIndex", "endIndex", metadata, "documentId", "createdAt")
        VALUES ${batch}
      `);
    }
    ...
    try {
      ...
      return rows.map((row: any) => {
        ...
        return {
          ...
          metadata: {
            ...metadata,
            documentId: metadata.documentId || row.documentId,
            chunkIndex: metadata.chunkIndex ?? row.chunkIndex,
          },
        };
      });
    } catch (error) {
