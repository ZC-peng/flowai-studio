...
export class RAGService {
  ...
  private async processAndEmbedDocument(documentId: string, content: string, knowledgeBaseId: string): Promise<void> {
    ...
    // 异步处理完成后再次失效缓存；否则上传接口写入的 processing 状态可能
    // 在 L1/L2 中保留数分钟，前端看不到 completed 和新分块。
    await this.invalidateKBCache(kb.userId, knowledgeBaseId);
    await this.cacheService.delete(`${CachePrefix.DOCUMENT}:chunks:${documentId}`);
    await this.cacheService.deleteByPrefix(
      `${CachePrefix.KNOWLEDGE_BASE_RETRIEVAL}:${knowledgeBaseId}`,
    );
  ...
  async retrieve(
    ...
    if (!query?.trim()) {
      throw new BadRequestException('Retrieval query must not be empty');
    }

    // 1. 获取知识库配置，同时在任何缓存/检索发生前校验租户所有权。
    const kb = await this.findKnowledgeBaseById(userId, knowledgeBaseId);
    ...
    const cacheKey =
      `${CachePrefix.KNOWLEDGE_BASE_RETRIEVAL}:${knowledgeBaseId}:` +
      `${this.hashQuery(query)}:${retrievalMode}:${effectiveTopK}:${vectorWeight}:${rrfK}`;
    ...
    if (cachedResults !== null) {
      ...
      return cachedResults as RAGRetrievalResult[];
    }
  ...
  private async applyReranker(
    ...
    try {
      ...
      const originalById = new Map(results.map((result) => [result.id, result]));

      // Reranker 只负责排序和相关性打分；引用信息必须从原检索结果保留下来。
      return rerankResult.results.map((reranked) => {
        const original = originalById.get(reranked.id);
        return {
          id: reranked.id,
          content: reranked.content,
          documentId: original?.documentId || String(reranked.metadata?.documentId || ''),
          documentName: original?.documentName || 'Unknown',
          chunkIndex: original?.chunkIndex ?? this.readChunkIndex(reranked.metadata),
          similarity: reranked.relevanceScore,
          source: original?.source || 'vector',
          originalSimilarity: original?.similarity ?? reranked.originalScore,
          rerankerScore: reranked.relevanceScore,
          metadata: original?.metadata || reranked.metadata,
          ...(original?.vectorScore !== undefined ? { vectorScore: original.vectorScore } : {}),
          ...(original?.keywordScore !== undefined ? { keywordScore: original.keywordScore } : {}),
          ...(original?.vectorRank !== undefined ? { vectorRank: original.vectorRank } : {}),
          ...(original?.keywordRank !== undefined ? { keywordRank: original.keywordRank } : {}),
        };
      });
    } catch (error) {
      this.logger.warn(
  ...
  private async retrieveVector(query: string, knowledgeBaseId: string, topK: number, kb: any): Promise<any[]> {
    ...
    return this.enrichResultsWithDocNames(
      searchResults.map((result) => ({ ...result, source: 'vector' as const })),
    );
  }
  ...
  private async retrieveHybrid(
    ...
    if (vResults.length === 0) {
      return this.enrichResultsWithDocNames(
        kResults.map((r) => ({ id: r.id, content: r.content, similarity: r.score, metadata: r.metadata, source: r.source }))
      );
    }
    if (kResults.length === 0) {
      return this.enrichResultsWithDocNames(
        vResults.map((r) => ({ id: r.id, content: r.content, similarity: r.score, metadata: r.metadata, source: r.source }))
      );
    }
  ...
  private async enrichResultsWithDocNames(
    ...
    return results.map((result) => ({
      ...
      chunkIndex: this.readChunkIndex(result.metadata),
      similarity: result.similarity,
      source: result.source || 'vector',
      metadata: result.metadata,
      ...(result.originalSimilarity !== undefined ? { originalSimilarity: result.originalSimilarity } : {}),
      ...(result.rerankerScore !== undefined ? { rerankerScore: result.rerankerScore } : {}),
      // 混合检索附加字段
  ...
  private readChunkIndex(metadata?: Record<string, any>): number | null {
