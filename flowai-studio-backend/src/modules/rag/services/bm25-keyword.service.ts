...
export class BM25KeywordService {
  ...
  async search(params: BM25SearchParams): Promise<BM25SearchResult[]> {
    ...
    try {
      ...
      if (rows.length === 0) {
        // PostgreSQL 默认 english/simple 配置无法可靠切分连续中文；零结果时
        // 使用受知识库范围约束的 LIKE 召回，保证 Hybrid 仍有关键词通道。
  ...
  /**
   * 英文按词切分；连续中文使用二元字符片段，避免把整句当成一个词而零召回。
   * 限制为 24 个去重片段，控制 SQL 长度和回退查询成本。
   */
  private extractFallbackKeywords(query: string): string[] {
    const keywords: string[] = [];
    const segments = query.match(/[a-zA-Z0-9_-]+|[\u3400-\u9fff]+/g) || [];

    for (const segment of segments) {
      if (/^[\u3400-\u9fff]+$/.test(segment) && segment.length > 2) {
        for (let index = 0; index < segment.length - 1; index++) {
          keywords.push(segment.slice(index, index + 2));
        }
      } else {
